...
public class Stages
{
	...
	public static Stage deploySnapshot() {
		String template =
				"docker-compose --no-ansi run --rm maven-gm mvn -U --batch-mode clean compile assembly:assembly deploy -DskipTests=true -P{profile} -Dmaven.wagon.http.ssl.insecure=true -Dmaven.wagon.http.ssl.allowall=true -Dmaven.wagon.http.ssl.ignore.validity.dates=true";
		ScriptTask deployTask = new ScriptTask()
				.inlineBody( String.join( "\n","# THIS SCRIPT IS GENERATED BY BAMBOO SPECS\nset -e\n", buildMavenDeployToNexusAndSonatypeCommand( template ) ))
