...
public class ImageServiceGetVariantSynchronizationTest {
    ...
    public void successfulSimultaneousGeneration() throws InterruptedException, ExecutionException, IOException {
        ...
        Answer<InMemoryImageSource> firstImageAnswer = new DelayedTransformResult(lock, inMemoryImageSource("IMAGE1"));
        Answer<InMemoryImageSource> secondImageAnswer = new DelayedTransformResult(lock, inMemoryImageSource("IMAGE2"));
        ...
        verify(imageTransformService, times(1)).modify(eq(testResults.getFirstOriginalImageSource()), anyInt(),
                anyInt(), anyInt(), anyInt(), anyInt(), anyInt(), anyInt(), anyInt(), eq(ImageType.PNG));
    ...
    private TestResults runTest(int threadsPerImage, Answer<InMemoryImageSource> firstImageAnswer, Answer<InMemoryImageSource> secondImageAnswer) throws InterruptedException {
        ...
        when(imageTransformService.modify(eq(firstOriginalImageSource), anyInt(), anyInt(), anyInt(), anyInt(),
                anyInt(), anyInt(), anyInt(), anyInt(), eq(ImageType.PNG))).thenAnswer(firstImageAnswer);
        when(imageTransformService.modify(eq(secondOriginalImageSource), anyInt(), anyInt(), anyInt(), anyInt(),
                anyInt(), anyInt(), anyInt(), anyInt(), eq(ImageType.PNG))).thenAnswer(secondImageAnswer);
        ...
        for (int i = 0; i < threadsPerImage; ++i) {
            firstImageFutures.add(executorService.submit(new GetVariantImageCallable(imageService, firstImage, context, imageResolution, imageVariant)));
            secondImageFutures.add(executorService.submit(new GetVariantImageCallable(imageService, secondImage, context, imageResolution, imageVariant)));
        }

        return new TestResults(firstImageFutures, secondImageFutures, firstOriginalImageSource, secondOriginalImageSource);
    }
    ...
    private static class TestResults {
        ...
        public TestResults(List<Future<StreamImageSource>> firstImageFutures, List<Future<StreamImageSource>> secondImageFutures, StreamImageSource firstOriginalImageSource, StreamImageSource secondOriginalImageSource) {
