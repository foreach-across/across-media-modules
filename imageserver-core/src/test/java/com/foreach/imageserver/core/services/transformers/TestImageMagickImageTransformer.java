...
public class TestImageMagickImageTransformer extends AbstractImageTransformerTest {
    ...
    public void croppedEpsImageShouldScaleInHighQuality() {
        ...
        modifier.getVariant().setWidth(1500);
        modifier.getVariant().setHeight(1250);
        modifier.getVariant().setDensity(5);
        modify("crop+x5", image, modifier, ImageTransformerPriority.PREFERRED, true);

        // Non uniform scale of the crop
        modifier.getVariant().setWidth(1500);
        modifier.getVariant().setHeight(500);
        modifier.getVariant().setDensity(5, 2);
    ...
    public void epsImageShouldScaleInHighQuality() {
        ...
        // uniform 3 times larger should be sharp
        modifier.getVariant().setWidth(130 * 3);
        modifier.getVariant().setHeight(104 * 3);
        modifier.getVariant().setDensity(3);
        modify("x3", image, modifier, ImageTransformerPriority.PREFERRED, true);

        // 10 times larger should also be sharp
        modifier.getVariant().setWidth(130 * 10);
        modifier.getVariant().setHeight(104 * 10);
        modifier.getVariant().setDensity(10);
        modify("x10", image, modifier, ImageTransformerPriority.PREFERRED, true);

        // Non uniform scales
        modifier.getVariant().setWidth(130 * 3);
        modifier.getVariant().setHeight(104 * 6);
        modifier.getVariant().setDensity(3, 6);
        modify("x3x6", image, modifier, ImageTransformerPriority.PREFERRED, true);

        modifier.getVariant().setWidth(130 * 10);
        modifier.getVariant().setHeight(104 * 3);
        modifier.getVariant().setDensity(10, 3);
    ...
    private ImageModification scale(ImageTestData image, ImageType output, float delta) {
        return scale(image, output, delta, delta);
    }

    private ImageModification scale(ImageTestData image, ImageType output, float deltaWidth, float deltaHeight) {
        ImageModification mod = new ImageModification();
        mod.getVariant().setStretch(true);
        mod.getVariant().setOutput(output);
        mod.getVariant().setWidth(Math.round(image.getDimensions().getWidth() * deltaWidth));
        mod.getVariant().setHeight(Math.round(image.getDimensions().getHeight() * deltaHeight));

        return mod;
    }

    private ImageModification crop(ImageType output, Crop crop) {
        return cropAndScale(output, crop, 1f);
    }

    private ImageModification cropAndScale(ImageType output, Crop crop, float delta) {
        return cropAndScale(output, crop, delta, delta);
    }

    private ImageModification cropAndScale(ImageType output, Crop crop, float deltaWidth, float deltaHeight) {
        ImageModification mod = new ImageModification();
        mod.getVariant().setStretch(true);
        mod.getVariant().setOutput(output);
        mod.setCrop(crop);
        mod.getVariant().setWidth(Math.round(crop.getWidth() * deltaWidth));
        mod.getVariant().setHeight(Math.round(crop.getHeight() * deltaHeight));
