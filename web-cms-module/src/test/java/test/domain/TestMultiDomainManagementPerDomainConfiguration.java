...
public class TestMultiDomainManagementPerDomainConfiguration extends AbstractMultiDomainTest
{
	@Test
	public void domainPropertyShouldBeHiddenForAllTypes() {
		entityRegistry.getEntities()
		              .stream()
		              .filter( cfg -> WebCmsDomainBound.class.isAssignableFrom( cfg.getEntityType() ) )
		              .forEach( cfg -> assertDomainPropertyHidden( cfg.getEntityType() ) );
	}
	...
	@Test
	public void defaultDomainBoundObjectOptionQueryIsFilteredOnVisibleDomains() {
		Stream.of( WebCmsArticle.class, WebCmsPage.class, WebCmsArticleType.class )
		      .forEach( type -> assertOptionQuery( type, "domain in (visibleDomains())" ) );

		assertNull( entityRegistry.getEntityConfiguration( WebCmsImage.class ).getAttribute( EntityAttributes.OPTIONS_ENTITY_QUERY ) );
	}
	...
	public void entityPropertiesReferringDomainBoundObjectHaveTheirCustomOptionQueryAppended() {
		EntityPropertyRegistry propertyRegistry = entityRegistry.getEntityConfiguration( WebCmsArticle.class )
		                                                        .getPropertyRegistry();
		assertEquals(
				"(published = TRUE) and (domain in (visibleDomains()))",
				propertyRegistry
