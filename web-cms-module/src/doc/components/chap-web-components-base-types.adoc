...
| container
| Container of multiple ordered components.
| container
| `ContainerWebCmsComponentModel`

| fixed-container
| Container of a fixed set of components.
| fixed-container
| `ContainerWebCmsComponentModel`
...
NOTE: The availability of the `WebCmsImage` asset and component infrastructure depends on the presence of other bean (eg an `ImageServerClient` bean).