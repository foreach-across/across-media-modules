...
when specified, only that scope will be queried unless search-parent-scopes is explicitly present
...
Otherwise adding the attribute will ensure that parent scopes are always searched, even if an explicit wcm:scope was specified.
...
Auto-creationg of components from markup
