...
var Cropper = {
	...
	adjustInitialSelection : function(e){
		...
		if(this.shiftKey && this.ratio > 0){

            var selectionRatio = width / height;

            // Pressing shift enforces a square selection.
			if(this.ratio < selectionRatio){
                var oldwidth = width;
                width = height * this.ratio;
                if(invertedHor) left += oldwidth - width;
            }
			else if(this.ratio > selectionRatio){
                var oldheight = height;
				height = width / this.ratio;
                if(invertedVer) top += oldheight - height;
            }
		}
    ...
    select : function (x, y, w, h)
    {
        ...
        this.selection.setLeft(x);
        this.selection.setTop(y);
        this.selection.setWidth(w);
        this.selection.setHeight(h);

        this.selection.draw(x, y, w, h);
        var shieldElm = document.getElementById(this.idShield);
        shieldElm.style.visibility = "visible";
	    this.selection.setCursor("default");
        this.setForm(x, y, w, h);
    },
    ...
    setForm : function(selectionX, selectionY, selectionW, selectionH){
		...
		if(arguments.length > 2){
            ...
            //document.getElementById("cropWlabel").innerHTML = selectionW;
            //document.getElementById("cropHlabel").innerHTML = selectionH;
        }

        // Stijn: dirty way to trigger something on the original page
		// document.getElementById("cropXlabel").innerHTML = selectionX;
        //document.getElementById("cropYlabel").innerHTML = selectionY;
	...
	formSetX : function(left){
		...
		if(error){
            ...
            //document.getElementById("cropXlabel").innerHTML = left;
            if(typeof width == "number" && left + width > this.imgW)
            {
                width = this.imgW - left;
                document.getElementById("cropForm").cropW.value = width;
                //document.getElementById("cropWlabel").innerHTML = width;
            }
	...
	formSetY : function(top){
		...
		if(error){
            ...
            //document.getElementById("cropYlabel").innerHTML = top;
            if(typeof height == "number" && top + height > this.imgH)
            {
                height = this.imgH - top;
                document.getElementById("cropForm").cropH.value = height;
                //document.getElementById("cropHlabel").innerHTML = height;
            }
	...
	formSetW : function(width){
		...
		if(error){
			...
			document.getElementById("cropForm").cropW.value = width;
            //document.getElementById("cropWlabel").innerHTML = width;
            if(typeof left == "number" && left + width > this.imgW)
            {
                left = this.imgW - width;
                document.getElementById("cropForm").cropX.value = left;
                //document.getElementById("cropXlabel").innerHTML = left;
            }
	...
	formSetH : function(height){
		...
		if(error){
			...
			document.getElementById("cropForm").cropH.value = height;
            //document.getElementById("cropHlabel").innerHTML = height;
            if(typeof top == "number" && top + height > this.imgH)
            {
                top = this.imgH - height;
                document.getElementById("cropForm").cropY.value = top;
                //document.getElementById("cropYlabel").innerHTML = top;
            }
    ...
    crop : function(ratio)
    {
        ...
        var selectionRatio = w / h;

        if (ratio > selectionRatio)
        {
            var newH = w / ratio;
            var y = this.selection.getTop();
            var yoffset = (h - newH) / 2;
            this.formSetH (newH);
            this.formSetY(y + yoffset);
        }
        else if (ratio < selectionRatio)
        {
            var newW = h * ratio;
            var x = this.selection.getLeft();
            var xoffset = (w - newW) / 2;
            this.formSetW( newW );
            this.formSetX(x + xoffset);
        }
    },

    cropToAspectRatio : function()
    {
        if (this.ratio <= 0)
        {
            return;
    ...
    extendToAspectRatio : function()
    {
        ...
        if (this.ratio > selectionRatio)
        {
            var newW = h * this.ratio;

            if (newW > imageW)
            {
                alert('can not extend across image boundaries');
                return;
            }

            var rightX = imageW - newW;
            var leftX = Math.max(0,x - (newW - w) / 2);

            this.formSetX(Math.min(leftX,rightX));
            this.formSetW(newW);
        }
        else if (this.ratio < selectionRatio)
        {
            var newH = w / this.ratio;
            ...
            var bottomY = imageH - newH;
            var upperY = Math.max(y - (newH - h) / 2,0);

            this.formSetY(Math.min(bottomY,upperY));
            this.formSetH( newH );
        }
    ...
    selectAll : function()
    {
        this.select(0,0,this.imgW,this.imgH);
    },

    selectMax : function(ratio)
    {
        this.selectAll();
        this.crop(ratio);
    },

    selectMaxAspectRatio : function()
    {
        if (this.ratio <= 0)
        {
            return;
        }
        this.selectMax(this.ratio);
